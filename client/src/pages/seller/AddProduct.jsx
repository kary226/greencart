...
const AddProduct = () => {
    ...
    const convertVariantsToApi = () => {
        ...
        colors.forEach(color => {
            // ✅ Si la couleur n'a pas de tailles, on ajoute un stock par défaut
            if (color.sizes.length === 0) {
                variants.push({
                    color: color.color,
                    colorCode: color.colorCode,
                    size: null,
                    stock: variantStockInput ? Number(variantStockInput) : 0,
    ...
    const onSubmitHandler = async (event) => {
        ...
        if (productMode === 'simple') {
            // ✅ Mode simple : juste un stock
            variants = [];
        } else if (productMode === 'variants') {
            // ✅ Mode variantes : couleurs + tailles (ou juste des couleurs avec stock)
            if (colors.length === 0) {
        ...
        // ✅ Ajouter le stock pour le mode simple
        if (productMode === 'simple') {
            productData.stock = simpleStock ? Number(simpleStock) : 0;
        }
            ...
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                ...
                <div className="border-t pt-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <button type="button" onClick={() => setProductMode('simple')} className={`py-2.5 px-3 rounded-lg text-sm font-medium border transition ${productMode === 'simple' ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 border-gray-300 hover:border-gray-400'}`}>
                            📦 Produit simple
                        </button>
                        <button type="button" onClick={() => setProductMode('variants')} className={`py-2.5 px-3 rounded-lg text-sm font-medium border transition ${productMode === 'variants' ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 border-gray-300 hover:border-gray-400'}`}>
                            🎨 Couleurs + Tailles
                        </button>
                    </div>
                    <p className="text-xs text-gray-400 mt-2">
                        {productMode === 'simple' && "Un produit avec un stock unique. Pas de couleurs ni de tailles."}
                        {productMode === 'variants' && "Ajoutez des couleurs et des tailles. La couleur est obligatoire, la taille est optionnelle."}
                    </p>
                </div>
                ...
                {productMode === 'simple' && (
                    <div className="flex flex-col gap-3 max-w-md">
                        <div className="flex flex-col gap-1">
                            ...
                            <input onChange={(e) => setSimpleStock(e.target.value)} value={simpleStock} type="number" min="0" placeholder="Quantité disponible" className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500/40" required />
                        </div>
                    </div>
                )}
                ...
                {productMode === 'variants' && (
                    <div className="flex flex-col gap-3 max-w-md">
                                        ...
                                        <div key={colorIndex} className="border border-gray-200 rounded-lg overflow-hidden">
                                            <button type="button" onClick={() => setOpenColorIndex(isOpen ? null : colorIndex)} className="w-full flex items-center justify-between px-3 py-2.5 bg-gray-50 hover:bg-gray-100 transition">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {color.sizes.length > 0 && <span className="text-xs text-green-600 ml-1">Stock total: {totalStock}</span>}
                                                </div>
                                            ...
                                            {isOpen && (
                                                <div className="px-3 py-3 border-t border-gray-200 bg-white">
                                                    ...
                                                    <div className="flex gap-2 mb-2">
                                                        <input value={variantSizeInput} onChange={e => setVariantSizeInput(e.target.value)} type="text" placeholder="Taille (optionnel)" className="flex-1 border border-gray-200 rounded-lg px-3 py-1.5 text-sm" />
                                                        <input value={variantStockInput} onChange={e => setVariantStockInput(e.target.value)} type="number" placeholder="Stock" className="w-20 border border-gray-200 rounded-lg px-3 py-1.5 text-sm" />
                                                    </div>
                                                    <div className="flex gap-2 mb-2">
                                                        <input value={variantPriceInput} onChange={e => setVariantPriceInput(e.target.value)} type="number" placeholder="Prix (optionnel)" className="flex-1 border border-gray-200 rounded-lg px-3 py-1.5 text-sm" />
                                                        <input value={variantOfferPriceInput} onChange={e => setVariantOfferPriceInput(e.target.value)} type="number" placeholder="Promo (optionnel)" className="flex-1 border border-gray-200 rounded-lg px-3 py-1.5 text-sm" />
                                                    </div>
                                                    <button type="button" onClick={() => addSizeToColor(colorIndex)} className="w-full py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 transition">
                        ...
                        {showColorForm && (
                            <div className="bg-gray-50 p-3 rounded-lg space-y-3 border border-gray-200">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium text-primary">{editingColorIndex !== null ? 'Modifier la couleur' : 'Nouvelle couleur'}</span>
                                    <button type="button" onClick={cancelColorForm} className="text-xs text-gray-400 hover:text-gray-600">Annuler</button>
                                </div>
                                <div className="flex gap-2 items-center">
                                    <input value={colorInput} onChange={e => setColorInput(e.target.value)} type="text" placeholder="Couleur (ex: Rouge)" className="flex-1 outline-none py-2 px-3 rounded border border-gray-300 text-sm" />
                                    <input value={colorCodeInput} onChange={e => setColorCodeInput(e.target.value)} type="color" className="w-12 h-10 rounded border border-gray-300 cursor-pointer" />
                                </div>
                                <div>
                                    <label className="text-xs text-gray-600 mb-1 block">Position de départ dans les photos (0 = première photo)</label>
                                    <input value={startImageIndexInput} onChange={e => setStartImageIndexInput(Number(e.target.value))} type="number" min="0" placeholder="Ex: 0 pour Rouge, 3 pour Bleu" className="w-full outline-none py-2 px-3 rounded border border-gray-300 text-sm" />
                        ...
                        <p className="text-xs text-gray-400 mt-1">💡 La couleur est obligatoire. La taille est optionnelle (laissez vide pour un produit sans taille).</p>
                    </div>
                )}
