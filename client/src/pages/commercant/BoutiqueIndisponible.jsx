...
const BoutiqueIndisponible = ({ erreur, onRetry }) => {
    ...
    return (
        <div className="bg-white rounded-2xl border border-blush-200 p-12 text-center max-w-lg mx-auto">
            <Store className="mx-auto text-blush-400 mb-3" size={40} />
            <h3 className="text-base font-medium text-gray-800">Boutique indisponible</h3>
            ...
            {erreur && (
                <p className="text-xs text-gray-400 mt-2 break-words">Détail : {erreur}</p>
            )}
            <button
                onClick={reessayer}
                disabled={enCours}
                className="mt-5 inline-flex items-center gap-2 bg-burgundy-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-burgundy-700 transition disabled:opacity-50"
            >
