...
export const getAdvancedKPIs = async (req, res) => {
    try {
        ...
        const data = await withCache(cacheKey, 60, async () => {
            // ─── Couverture d'audit ─────────────────────────────────────
            // Taux d'actions journalisées sur les actions sensibles totales
            ...
            // ─── Reste-à-migrer seller ──────────────────────────────────
            // Comptage des routes encore protégées par authSeller
            // Estimation basée sur le nombre de routes encore actives
            const routesSeller = [
                'productRoute',
                'orderRoute',
                'bannerRoute',
            ...
            const resteAMigrer = routesSeller.length; // estimation
