...
staffRouter.post('/login', authLimiter, staffLogin);
staffRouter.post('/activation/:token', activationLimiter, activateAccount);
staffRouter.get('/is-auth', authStaff, isStaffAuth);
staffRouter.get('/logout', staffLogout);

// ---- Gestion des comptes (admin uniquement) ----
staffRouter.post('/invitations', authStaff, requireRole('admin'), createInvitation);
